...
_Page({
    ...
    zutils.get(app, "api/user/vip-info", function(res) {
      ...
      if (_data.level != "N") {
        let info =
          ...
          "会员 ۰ " +
          _data.expires;
        if (_data.is_expired == true)
          info = _data.subject + _data.level + "会员 ۰ 已过期";
        that.setData({
          vipLevel: info,
  ...
  __loadBuy: function(s) {
    let that = this;
    zutils.get(app, "api/user/buy-vip-pre?subject=" + (s || ""), function(
      res
    ) {
      let _data = res.data.data;
  ...
  buyNow: function() {
    ...
    let _url =
      "api/pay/create-buyvip?subject=" +
      this.__buydata.subject +
      "&tt=" +
      this.data.tt +
      "&coupon=" +
