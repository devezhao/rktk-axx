...
_Page({
    ...
    onShow: function(e) {
        ...
        if (
            zutils.array.in(app.GLOBAL_DATA.RELOAD_SUBJECT, "Home") ||
    ...
    __onLoad: function(cb, retry) {
        ...
        zutils.get(app, "api/user/infos", function(res) {
            ...
            if (!_data) {
                if (retry) {
                    _data = _my.getStorageSync("home.USER_INFOS");
                    console.warn(
                        "Use cache when request failed again : " +
            ...
            if (isVip) {
                ...
                zutils.get(app, "api/user/vip-info", function(res) {
                    ...
                    that.setData({
                        vipLevelFull:
                            "已开通" + _data.subject + _data.level + "会员"
                    });
                });
            }
        });
    ...
    userInfo: function() {
        if (this.data.longUid)
            this.setData({
                longUid: 0
            });
        else
            this.setData({
                longUid: this.__UID || 0
            });
    },
    iosBuyVipClick: 0,
    iosBuyVip: function() {
