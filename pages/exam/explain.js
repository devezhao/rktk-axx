...
_Page({
    ...
    onLoad: function(e) {
        ...
        if (viewSubject == true) {
            ...
            if (e.interactive == 1) {
                ...
                zutils.get(
                    app,
                    "api/user/can-interactive?subject=" + this.qosid,
                    function(res) {
                        that.canInteractive = res.data.data != "NO";

                        if (
                            that.data.interactiveMode == true &&
                            that.canInteractive == false
                        ) {
                            that.setData({
                                interactiveMode: false
                            });
        ...
        app.getUserInfo(function(u) {
            ...
            if (viewSubject == false) {
                ...
                zutils.get(
                    app,
                    "api/subject/subject-qids?subject=" + that.qosid,
                    function(res) {
                        that.__qids = res.data.data;
                        let idx = 1;

                        try {
                            idx = _my.getStorageSync(
                                "ExplainPageNo" + that.qosid
                            );
        ...
        this.turningAnimation = _my.createAnimation({
            duration: 200,
            timingFunction: "ease",
    ...
    __loadQuestion: function(idx) {
        ...
        zutils.get(
            app,
            "api/question/details?id=" + this.data.currentQuestionId,
            function(res) {
    ...
    __loadQuestionAfter: function(_data) {
        ...
        for (let i = 0; i < _data.answer_list.length; i++) {
            ...
            if (this.answerKey && this.answerKey.indexOf(item[0]) > -1)
                clazz += " selected";
            item[10] = clazz;
    ...
    interactiveAnswer: function(res) {
        ...
        if (
            this.__selectAnswer &&
            zutils.array.in(this.__selectAnswer, keyIndex)
        ) {
            return;
        }
    ...
    onShareAppMessage: function() {
        let d = app.warpShareData(
            "/pages/exam/explain?id=" + this.data.currentQuestionId
        );
        d.title =
            "#考题解析#" +
            this.data.question
