...
_Page({
  ...
  onLoad: function(e) {
    ...
    this.dtcardAnimation = _my.createAnimation({
      duration: 200,
      timingFunction: "ease",
      delay: 50
    });
  ...
  loadQuestion: function() {
    ...
    zutils.get(
      app,
      "api/question/list?subject=" +
      that.subjectId +
      "&restore=" +
      (that.duration > 0 ? that.examId : ""),
      function(res) {
  ...
  renderQuestion: function(s, e) {
    ...
    if (q._answers) {
      ...
      zutils.get(
        app,
        "api/question/get-answers?noloading&question=" +
        q.questionId +
        "&formId=" +
        formId,
        function(res) {
          ...
          for (var i = 0; i < data.length; i++) {
            ...
            if (
              _selected &&
              _selected.join(",").indexOf(dd.key) > -1
            ) {
              dd.clazz = "selected";
            }
          }
  ...
  answer: function(e) {
    ...
    this.questionList[this.data.seqCurrent]._selected = _selected; // console.log(JSON.stringify(_selected));

    let canNext =
      _selected.length == q.answerNum && that.examAutoNext == true;
    zutils.post(
      app,
      "api/exam/record?exam=" +
      that.examId +
      "&question=" +
      q.itemId +
      "&answer=" +
  ...
  fav: function(e) {
    ...
    zutils.post(
      app,
      "api/fav/toggle?question=" + q.questionId + "&formId=" + formId,
      function(res) {
  ...
  __finish: function(e) {
    ...
    zutils.post(
      app,
      "api/exam/finish?noloading&exam=" +
      that.examId +
      "&formId=" +
      formId,
      function(res) {
