...
_Page({
    ...
    loadResult: function(checkKeep) {
        ...
        zutils.get(app, "api/exam/result?exam=" + that.examId, function(res) {
            ...
            that.setData({
                ...
                scope_tips:
                    scope_percent < 0.6 ? "本次答题未通过" : "本次答题通过",
            ...
            that.subjectId = _data.subject_id; // Keep

            if (checkKeep == true) {
                zutils.get(app, "api/acts/keep-days-v2?noloading", function(
                    res
                ) {
                    let dd = res.data;
    ...
    __drawCircle: function(p) {
        var ctx = _my.createCanvasContext("circle-scope");
