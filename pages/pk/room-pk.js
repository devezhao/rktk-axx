...
_Page({
    ...
    onLoad: function(e) {
        ...
        this.__initAudio("lost.wav", false); // test result
        // this.setData({ fooScope: 220, barScope: 140 });
        // this.__complete();
    },
    renderQuestion: function() {
        let qno =
            "第" + ["一", "二", "三", "四", "五"][this.questionIdx] + "题";
        if (this.questionIdx == 4)
            qno =
                '最后一题<div style="margin-top:-4px;color:#ff0">分数加倍</div>';
        this.setData({
    ...
    checkToNext: function() {
        let q = this.questions[this.questionIdx]; // 都回答了
    ...
    __initAudio: function(file, loop) {
        this.__audio[file] = _my.createInnerAudioContext();
        this.__audio[file].src =
            "https://cdn.chinaruankao.com/a/wxx/pk/" + file;
